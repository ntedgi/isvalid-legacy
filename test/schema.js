...
describe('schema', function () {
    describe('formalizer', function () {
        ...
        it('[async] should come back with no error and match set if match is RegExp.', function (done) {
            const s = {type: String, match: /test/}
            formalize(s, function (x) {
                console.log(x);
        ...
        testSyncAndAsync('should come back with no error and match set if match is RegExp.', {
            type: String,
            match: /test/
        }, function (s) {
            expect(s).to.have.property('match');
        ...
        testSyncAndAsync('should come back with custom wrapped in an array.', {
            custom: function () {
            }
        }, function (s) {
            expect(s).to.have.property('custom').to.be.an('array');
        });
        testSyncAndAsync('should come back with custom as an array.', {
            custom: [function () {
            }]
        }, function (s) {
            expect(s).to.have.property('custom').to.be.an('array');
